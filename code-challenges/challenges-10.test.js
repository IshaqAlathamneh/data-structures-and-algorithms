...
const count = (target, input) => {
  ...
  let newArr = input.map(result => {
    let inArr = result.filter(x => x == target)
    n += inArr.length;
  })
  return n;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filtered = input.map( x => {
   return x.filter(y => y%5 == 0 && typeof y == 'number')
  });
  
  let result = filtered.map(res => {
    return res.map(b => Math.pow(2,b));
  })
  return result;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filtered = data.filter(x => x.gender == 'male' || x.gender == 'female');
  let name = '';
   for (let i = 0; i < filtered.length; i++){
     if(i < filtered.length -1){
       name = name + `${filtered[i].name} and `;
     }else{
      name = name + `${filtered[i].name}`;
     }
   }
   return name;
};
...
let findShortest = (data) => {
  // Solution code here...
  let g = data[0].height;
let sorted = data.filter(x => {
  if(parseInt(x.height) < parseInt(g)){
    g = x.height;
  }
  return parseInt(x.height) <= parseInt(g);
  })
